...
This creates `sdata-0.6.7-1.src.rpm` which can be built with `rpmbuild
...
This creates `sdata-0.6.7-slackbuild.tar.gz` containing the SlackBuild script,
...
```sh
scripts/bump-version.sh 0.6.7 "Add spreadsheet formula evaluation and multi-sheet support."
```
...
```
$ sdata
SData Statistical Interpreter version 0.6.7
