...
This creates `sdata-0.2.1-1.src.rpm` which can be built with `rpmbuild
...
This creates `sdata-0.2.1-slackbuild.tar.gz` containing the SlackBuild script,
...
This creates `sdata-0.2.1.pkg`. The `sdata` binary must already be built
...
```
$ sdata
SData Statistical Interpreter version 0.2.1
