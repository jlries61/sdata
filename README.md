...
This creates `sdata-0.6.0-1.src.rpm` which can be built with `rpmbuild
...
This creates `sdata-0.6.0-slackbuild.tar.gz` containing the SlackBuild script,
...
This creates `sdata-0.6.0.pkg`. The `sdata` binary must already be built
...
```sh
scripts/bump-version.sh 0.6.0 "Add spreadsheet formula evaluation and multi-sheet support."
```
...
```
$ sdata
SData Statistical Interpreter version 0.6.0
