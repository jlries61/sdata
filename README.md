...
This creates `sdata-0.3.3-1.src.rpm` which can be built with `rpmbuild
...
This creates `sdata-0.3.3-slackbuild.tar.gz` containing the SlackBuild script,
...
This creates `sdata-0.3.3.pkg`. The `sdata` binary must already be built
...
```
$ sdata
SData Statistical Interpreter version 0.3.3
