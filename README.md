...
This creates `sdata-0.6.8-1.src.rpm` which can be built with `rpmbuild
...
This creates `sdata-0.6.8-slackbuild.tar.gz` containing the SlackBuild script,
...
This creates `sdata-0.6.8.pkg`. The `sdata` binary must already be built
...
This creates `sdata-0.6.8-x64.msi`, a per-machine x64 installer that places
...
Install the resulting MSI with `msiexec /i sdata-0.6.8-x64.msi`, or by
...
```sh
scripts/bump-version.sh 0.6.8 "Add spreadsheet formula evaluation and multi-sheet support."
```
...
```
$ sdata
SData Statistical Interpreter version 0.6.8
