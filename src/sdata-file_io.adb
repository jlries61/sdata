...
package body SData.File_IO is
   ...
   begin
      while Child /= null loop
         if Node_Type (Child) = Text_Node then Append (Res, Node_Value (Child));
         elsif Node_Type (Child) = Element_Node then Append (Res, Get_Text (Child)); end if;
      ...
      begin
         ...
         begin
            loop
               ...
               if Pos = 0 then Res (Idx) := To_Unbounded_String (Trim (L (Start .. L'Last), Ada.Strings.Both)); exit;
               else Res (Idx) := To_Unbounded_String (Trim (L (Start .. Pos - 1), Ada.Strings.Both)); Start := Pos + 1; Idx := Idx + 1; end if;
            end loop; return Res;
         end;
      end Split;
            ...
            if I <= Names'Length then
               ...
               begin
                  if Val_Str = "" or Val_Str = "." then Val := (Kind => Val_Missing);
                     ...
                     begin Val := (Kind => Val_Numeric, Num_Val => Float'Value (Val_Str));
                     exception when others => Val := (Kind => Val_String, Str_Val => Val_Str & (1 .. 1024 - Val_Str'Length => ' '), Str_Len => Val_Str'Length); end;
                  end if;
   ...
   begin
      ...
      if Has_Header then
         declare
            Headers : constant String_Array := Split (To_String (Header_Line)); Names : String_List (1 .. Headers'Length);
         ...
         begin
            ...
            for I in Headers'Range loop
               declare Name : constant String := Safe_Name (To_String (Headers (I)), "COL" & Trim (I'Img, Ada.Strings.Both));
         ...
         for R in 1 .. Row_Count loop
            for C in Col_Names'Range loop
               ...
               begin
                  ...
                  elsif Val.Kind = Val_Integer then Put (File, Trim (Val.Int_Val'Img, Ada.Strings.Both));
   ...
   procedure Parse_ODF (File_Name : String) is
      ...
      Tables, Rows, Text_Nodes : DOM.Core.Node_List; Success : Boolean;
      Col_Names : String_List_Access; use DOM.Core; use DOM.Core.Nodes; Info : Zip.Zip_Info;
   begin
      Zip.Load (Info, File_Name); UnZip.Extract (from => Info, what => "content.xml", rename => Temp_XML);
      Input_Sources.File.Open (Temp_XML, Input); DOM.Readers.Parse (Reader, Input); Doc := DOM.Readers.Get_Tree (Reader); Input_Sources.File.Close (Input);
      Tables := DOM.Core.Documents.Get_Elements_By_Tag_Name (Doc, "table:table");
      if Length (Tables) = 0 then GNAT.OS_Lib.Delete_File (Temp_XML, Success); raise Program_Error with "No tables found in ODS"; end if;
      Rows := DOM.Core.Elements.Get_Elements_By_Tag_Name (DOM.Core.Element (Item (Tables, 0)), "table:table-row");
      Clear;
      if Length (Rows) > 0 then
         declare Header_Cells : Node_List := DOM.Core.Elements.Get_Elements_By_Tag_Name (DOM.Core.Element (Item (Rows, 0)), "table:table-cell");
         ...
         begin
            ...
            declare Ordered_Names : String_List (1 .. Max_Cols); Ordered_Types : array (1 .. Max_Cols) of Column_Type := (others => Col_String);
            begin
               Col_Idx := 1;
               for I in 0 .. Length (Header_Cells) - 1 loop
                  declare Cell_Node : constant Node := Item (Header_Cells, I); Name : Unbounded_String := Null_Unbounded_String;
                          ...
                          Repeat_Count : Positive := (if Repeat_Attr /= "" then Positive'Value (Repeat_Attr) else 1);
                  begin
                     Text_Nodes := DOM.Core.Elements.Get_Elements_By_Tag_Name (DOM.Core.Element (Cell_Node), "text:p");
                     if Length (Text_Nodes) > 0 then Name := To_Unbounded_String (Get_Text (Item (Text_Nodes, 0))); end if;
                     if Length (Rows) > 1 then
                        declare Data_Row_Cells : Node_List := DOM.Core.Elements.Get_Elements_By_Tag_Name (DOM.Core.Element (Item (Rows, 1)), "table:table-cell");
               ...
               for I in 1 .. Length (Rows) - 1 loop
                  declare Row_Node : constant Node := Item (Rows, I); Row_Repeat_Attr : constant String := DOM.Core.Elements.Get_Attribute (DOM.Core.Element (Row_Node), "table:number-rows-repeated");
                          Num_Row_Repeats : Positive := (if Row_Repeat_Attr /= "" then Positive'Value (Row_Repeat_Attr) else 1);
                          Data_Cells : Node_List := DOM.Core.Elements.Get_Elements_By_Tag_Name (DOM.Core.Element (Item (Rows, I)), "table:table-cell");
                  begin
                        ...
                        begin
                           for J in 0 .. Length (Data_Cells) - 1 loop
                              declare Cell : constant Node := Item (Data_Cells, J);
                                      ...
                                      Repeat_Count : Positive := (if Repeat_Attr /= "" then Positive'Value (Repeat_Attr) else 1);
                              begin
                                 Text_Nodes := DOM.Core.Elements.Get_Elements_By_Tag_Name (DOM.Core.Element (Cell), "text:p");
      ...
      DOM.Core.Free (Rows); DOM.Core.Free (Tables); DOM.Readers.Free (Reader); GNAT.OS_Lib.Delete_File (Temp_XML, Success);
   ...
   procedure Parse_OOXML (File_Name : String) is
      ...
      SI_Nodes, T_Nodes, Rows_List, Cells_List, V_Nodes, IS_Nodes : DOM.Core.Node_List; Info : Zip.Zip_Info;
   begin
      ...
      begin
         ...
         for I in 0 .. Length (SI_Nodes) - 1 loop
            T_Nodes := DOM.Core.Elements.Get_Elements_By_Tag_Name (DOM.Core.Element (Item (SI_Nodes, I)), "t");
            if Length (T_Nodes) > 0 then Shared_Strings.Append (To_Unbounded_String (Get_Text (Item (T_Nodes, 0)))); else Shared_Strings.Append (Null_Unbounded_String); end if; DOM.Core.Free (T_Nodes);
         end loop;
         DOM.Core.Free (SI_Nodes); DOM.Readers.Free (Reader); GNAT.OS_Lib.Delete_File (Temp_Shared, Success);
      ...
      Rows_List := DOM.Core.Documents.Get_Elements_By_Tag_Name (Doc, "row");
      declare Col_Names : String_List_Access;
      begin
         Clear;
         if Length (Rows_List) > 0 then
            declare Header_Cells : Node_List := DOM.Core.Elements.Get_Elements_By_Tag_Name (DOM.Core.Element (Item (Rows_List, 0)), "c");
                    Ordered_Names : String_List (1 .. Length (Header_Cells)); Ordered_Types : array (1 .. Length (Header_Cells)) of Column_Type := (others => Col_String);
            begin
               for I in 0 .. Length (Header_Cells) - 1 loop
                  ...
                  begin
                     V_Nodes := DOM.Core.Elements.Get_Elements_By_Tag_Name (DOM.Core.Element (Cell_Node), "v");
                     if Length (V_Nodes) > 0 then
                        ...
                        IS_Nodes := DOM.Core.Elements.Get_Elements_By_Tag_Name (DOM.Core.Element (Cell_Node), "is");
                        if Length (IS_Nodes) > 0 then
                           T_Nodes := DOM.Core.Elements.Get_Elements_By_Tag_Name (DOM.Core.Element (Item (IS_Nodes, 0)), "t");
                     ...
                     if Length (Rows_List) > 1 then
                        declare Data_Cells : Node_List := DOM.Core.Elements.Get_Elements_By_Tag_Name (DOM.Core.Element (Item (Rows_List, 1)), "c");
                     ...
                     Add_Column (Ordered_Names (I + 1).all, Ordered_Types (I + 1)); DOM.Core.Free (V_Nodes);
                  end;
               end loop;
               for I in 1 .. Length (Rows_List) - 1 loop
                  Add_Row; Cells_List := DOM.Core.Elements.Get_Elements_By_Tag_Name (DOM.Core.Element (Item (Rows_List, I)), "c");
                  for J in 0 .. Length (Cells_List) - 1 loop
                     if J < Length (Header_Cells) then
                        declare Cell_Node : constant Node := Item (Cells_List, J); Type_Attr : constant String := DOM.Core.Elements.Get_Attribute (DOM.Core.Element (Cell_Node), "t");
                        begin
                           V_Nodes := DOM.Core.Elements.Get_Elements_By_Tag_Name (DOM.Core.Element (Cell_Node), "v");
                           if Length (V_Nodes) > 0 then
                              declare Val_Text : constant String := Get_Text (Item (V_Nodes, 0));
                              begin
                                 ...
                                 else begin Set_Value (Row_Count, Ordered_Names (J + 1).all, (Kind => Val_Numeric, Num_Val => Float'Value (Val_Text))); exception when others => null; end; end if;
                              end;
                           elsif Type_Attr = "inlineStr" then
                              IS_Nodes := DOM.Core.Elements.Get_Elements_By_Tag_Name (DOM.Core.Element (Cell_Node), "is");
                              if Length (IS_Nodes) > 0 then
