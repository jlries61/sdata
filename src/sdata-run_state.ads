...
--  Package SData.Run_State holds interpreter state that changes during
--  execution and is reset to defaults by the NEW command.  Separating it from
--  SData_Core.Config makes the boundary between startup configuration (set
--  once by the CLI) and per-run state (written by the interpreter) explicit.

with Ada.Strings.Unbounded; use Ada.Strings.Unbounded;
with SData_Core.Config;

package SData.Run_State is

   Save_File_Path      : String (1 .. SData_Core.Max_Path_Len)       := (others => ' ');
   Save_File_Len       : Natural                                       := 0;
   Save_File_Active    : Boolean                                       := False;
   Save_File_Fmt       : SData_Core.Config.Format_Type                := SData_Core.Config.CSV;
   Save_Sheet_Name     : String (1 .. SData_Core.Max_Sheet_Name_Len)  := (others => ' ');
   ...
   Options_CSVDLM      : String (1 .. SData_Core.Max_Delimiter_Len)  := (',' , others => ' ');
   Options_CSVDLM_Len  : Natural                                      := 1;
   Options_Header      : Boolean                                      := True;
   Options_SAVEOVERWRT : Boolean                                      := True;
   Options_TXTFMT      : String (1 .. SData_Core.Max_Delimiter_Len)  := "AUTO    ";
   Options_TXTFMT_Len  : Natural                                      := 4;
   Options_CHARSET     : String (1 .. SData_Core.Max_Charset_Len)    := (others => ' ');
   ...
   Save_DLM         : String (1 .. SData_Core.Max_Delimiter_Len)  := (',' , others => ' ');
   Save_DLM_Len     : Natural                                      := 1;
   Save_Header      : Boolean                                      := True;
   Save_Charset     : String (1 .. SData_Core.Max_Charset_Len)    := (others => ' ');
