...
**Annotation:** 2026-05-14 (v0.6.13) — GNATcheck rules added locally: `gnatcheck.rules` with two rules (`Recursive_Subprograms`, `Too_Many_Parameters:8`) and `make gnatcheck` target. Intentional exceptions documented with in-source `pragma Annotate` exemptions: mutual recursion in `Evaluate`/`Evaluate_Function` (AST traversal); `Open_Input` 9-parameter API (7 optional params with safe defaults). `gnatcheck` is NOT run in CI — the tool is not reliably available outside Debian/Ubuntu `gnat` packages and is absent on the primary development platform (openSUSE). `make gnatcheck` remains available for manual use on Debian-derivative hosts. No CI score change; §6 score remains **75**; total remains **619/800 (77.4%)**.
