...
| No fuzz coverage of the merge / RENAME surface | The fuzz drivers cover CSV, the lexer/parser, and ODF/OOXML, but not the `SData.Merge` / `SData.Transient_Table` path or `RENAME`-option syntax added since v0.8.0. A merge/option fuzz driver (or parser-driver seeds for `(KEEP=/DROP=/RENAME=)` and merge slash-options) would close this. Tracked as standards-review remediation #7. |
