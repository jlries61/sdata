...
| ~~4~~ | ~~Introduce `Interpreter_Context`; thread through data step chain~~ | Fowler, Feathers | — | ✅ *Done d2d40bc + subsequent — `Step_Context` (By_Vars, Deleted, BOG, EOG) threaded through entire data step chain; `Current_Record_Deleted` global eliminated; `BOG_Flag`/`EOG_Flag` moved to `Nav_Fns` body; no evaluator-level globals remain* |
