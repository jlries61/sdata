...
6. 498 `.cmd` integration tests in `tests/` (881 unit checks total)

All 498 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 498 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (498)
doc/
...
- Phase 6 (Testing): **ongoing** — 498 integration tests, 881 unit checks across 5 modules
