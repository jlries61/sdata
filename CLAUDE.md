...
6. 362 `.cmd` integration tests in `tests/` (863 unit checks total)

All 362 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 362 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (362)
doc/
  SOFTWARE_STANDARDS_REVIEW.md  -- living standards audit (annotated)
  adrs.md                     -- 53 ADRs (ADR-001 through ADR-053; contiguous)
...
Markdown; read directly. Documents 53 ADRs (ADR-001 through ADR-053,
...
- Phase 6 (Testing): **ongoing** — 362 integration tests, 863 unit checks across 5 modules
