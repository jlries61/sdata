...
6. 592 `.cmd` integration tests in `tests/` (885 unit checks total)

All 592 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 592 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (592)
doc/
...
- Phase 6 (Testing): **ongoing** — 592 integration tests, 885 unit checks across 5 modules
