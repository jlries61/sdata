...
6. 522 `.cmd` integration tests in `tests/` (881 unit checks total)

All 522 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 522 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (522)
doc/
  SOFTWARE_STANDARDS_REVIEW.md  -- living standards audit (annotated)
  adrs.md                     -- 68 ADRs (ADR-001 through ADR-068; contiguous)
...
Markdown; read directly. Documents 68 ADRs (ADR-001 through ADR-068,
...
- Phase 6 (Testing): **ongoing** — 522 integration tests, 881 unit checks across 5 modules
