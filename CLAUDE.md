...
6. 520 `.cmd` integration tests in `tests/` (881 unit checks total)

All 520 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 520 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (520)
doc/
  SOFTWARE_STANDARDS_REVIEW.md  -- living standards audit (annotated)
  adrs.md                     -- 67 ADRs (ADR-001 through ADR-067; contiguous)
...
Markdown; read directly. Documents 67 ADRs (ADR-001 through ADR-067,
...
- Phase 6 (Testing): **ongoing** — 520 integration tests, 881 unit checks across 5 modules
