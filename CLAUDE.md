...
6. 424 `.cmd` integration tests in `tests/` (881 unit checks total)

All 424 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 424 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (424)
doc/
...
- Phase 6 (Testing): **ongoing** — 424 integration tests, 881 unit checks across 5 modules
