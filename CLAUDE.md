...
6. 371 `.cmd` integration tests in `tests/` (863 unit checks total)

All 371 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 371 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (371)
doc/
...
- Phase 6 (Testing): **ongoing** — 371 integration tests, 863 unit checks across 5 modules
