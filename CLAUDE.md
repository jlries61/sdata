...
6. 372 `.cmd` integration tests in `tests/` (863 unit checks total)

All 372 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 372 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (372)
doc/
...
- Phase 6 (Testing): **ongoing** — 372 integration tests, 863 unit checks across 5 modules
