...
6. 410 `.cmd` integration tests in `tests/` (876 unit checks total)

All 410 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 410 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (410)
doc/
...
- Phase 6 (Testing): **ongoing** — 410 integration tests, 876 unit checks across 5 modules
