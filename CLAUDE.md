...
6. 428 `.cmd` integration tests in `tests/` (881 unit checks total)

All 428 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 428 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (428)
doc/
...
- Phase 6 (Testing): **ongoing** — 428 integration tests, 881 unit checks across 5 modules
