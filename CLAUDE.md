...
6. 527 `.cmd` integration tests in `tests/` (881 unit checks total)

All 527 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 527 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (527)
doc/
  SOFTWARE_STANDARDS_REVIEW.md  -- living standards audit (annotated)
  adrs.md                     -- 70 ADRs (ADR-001 through ADR-070; contiguous)
...
Markdown; read directly. Documents 70 ADRs (ADR-001 through ADR-070,
...
- Phase 6 (Testing): **ongoing** — 527 integration tests, 881 unit checks across 5 modules
