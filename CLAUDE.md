...
6. 599 `.cmd` integration tests in `tests/` (885 unit checks total)

All 599 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 599 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (599)
doc/
  SOFTWARE_STANDARDS_REVIEW.md  -- living standards audit (annotated)
  adrs.md                     -- 77 ADRs (ADR-001 through ADR-077; contiguous)
...
Markdown; read directly. Documents 77 ADRs (ADR-001 through ADR-077,
...
- Phase 6 (Testing): **ongoing** — 599 integration tests, 885 unit checks across 5 modules
