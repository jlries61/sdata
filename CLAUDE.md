...
6. 357 `.cmd` integration tests in `tests/` (863 unit checks total)

All 357 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 357 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (357)
doc/
...
- Phase 6 (Testing): **ongoing** — 357 integration tests, 863 unit checks across 5 modules
