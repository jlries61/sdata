...
6. 596 `.cmd` integration tests in `tests/` (885 unit checks total)

All 596 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 596 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (596)
doc/
  SOFTWARE_STANDARDS_REVIEW.md  -- living standards audit (annotated)
  adrs.md                     -- 76 ADRs (ADR-001 through ADR-076; contiguous)
...
Markdown; read directly. Documents 76 ADRs (ADR-001 through ADR-076,
...
- Phase 6 (Testing): **ongoing** — 596 integration tests, 885 unit checks across 5 modules
