...
2. `bin/sdata_unit_test` — `SData_Core.Table` / `Variables` / transient-table / merge / PDV (375)
...
6. 407 `.cmd` integration tests in `tests/` (876 unit checks total)

All 407 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 407 integration tests) and
`cd ~/Develop/data-vandal && make check` (data-vandal, 148 integration tests) still pass.
...
tests/
  ...
  *.cmd                       -- integration test scripts (407)
doc/
...
Markdown; read directly. Documents 57 ADRs (ADR-001 through ADR-057,
...
- Phase 6 (Testing): **ongoing** — 407 integration tests, 876 unit checks across 5 modules
