...
6. 198 `.cmd` integration tests in `tests/` (~733 unit checks total)

All 198 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 198 integration tests) and
...
- Phase 6 (Testing): **ongoing** — 198 integration tests, ~733 unit checks across 5 modules
