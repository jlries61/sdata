...
6. 421 `.cmd` integration tests in `tests/` (881 unit checks total)

All 421 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 421 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (421)
doc/
...
- Phase 6 (Testing): **ongoing** — 421 integration tests, 881 unit checks across 5 modules
