...
6. 510 `.cmd` integration tests in `tests/` (881 unit checks total)

All 510 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 510 integration tests) and
...
- Phase 6 (Testing): **ongoing** — 510 integration tests, 881 unit checks across 5 modules
