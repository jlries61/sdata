...
6. 597 `.cmd` integration tests in `tests/` (885 unit checks total)

All 597 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 597 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (597)
doc/
...
- Phase 6 (Testing): **ongoing** — 597 integration tests, 885 unit checks across 5 modules
