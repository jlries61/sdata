...
6. 383 `.cmd` integration tests in `tests/` (863 unit checks total)

All 383 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 383 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (383)
doc/
...
- Phase 6 (Testing): **ongoing** — 383 integration tests, 863 unit checks across 5 modules
