...
6. 365 `.cmd` integration tests in `tests/` (863 unit checks total)

All 365 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 365 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (365)
doc/
  SOFTWARE_STANDARDS_REVIEW.md  -- living standards audit (annotated)
  adrs.md                     -- 54 ADRs (ADR-001 through ADR-054; contiguous)
...
Markdown; read directly. Documents 54 ADRs (ADR-001 through ADR-054,
...
- Phase 6 (Testing): **ongoing** — 365 integration tests, 863 unit checks across 5 modules
