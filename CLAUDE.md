...
6. 477 `.cmd` integration tests in `tests/` (881 unit checks total)

All 477 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 477 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (477)
doc/
...
- Phase 6 (Testing): **ongoing** — 477 integration tests, 881 unit checks across 5 modules
