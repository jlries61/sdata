...
6. 591 `.cmd` integration tests in `tests/` (885 unit checks total)

All 591 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 591 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (591)
doc/
  SOFTWARE_STANDARDS_REVIEW.md  -- living standards audit (annotated)
  adrs.md                     -- 75 ADRs (ADR-001 through ADR-075; contiguous)
...
Markdown; read directly. Documents 75 ADRs (ADR-001 through ADR-075,
...
- Phase 6 (Testing): **ongoing** — 591 integration tests, 885 unit checks across 5 modules
