...
6. 525 `.cmd` integration tests in `tests/` (881 unit checks total)

All 525 integration tests must pass before committing. Never use `--no-verify`.
...
sdata-core and confirm both `make check` (sdata, 525 integration tests) and
...
tests/
  ...
  *.cmd                       -- integration test scripts (525)
doc/
  SOFTWARE_STANDARDS_REVIEW.md  -- living standards audit (annotated)
  adrs.md                     -- 69 ADRs (ADR-001 through ADR-069; contiguous)
...
Markdown; read directly. Documents 69 ADRs (ADR-001 through ADR-069,
...
- Phase 6 (Testing): **ongoing** — 525 integration tests, 881 unit checks across 5 modules
