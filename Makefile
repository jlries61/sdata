...
srpm: clean
	...
	@{ \
		if [ -z "$$(git status --untracked-files=no --porcelain)" ]; then \
			git archive --format=tar --prefix=sdata-0.2.2/ HEAD | gzip > sdata-0.2.2.tar.gz; \
		else \
	...
	@mv sdata-0.2.2.tar.gz rpmbuild/SOURCES/
	...
	@mv rpmbuild/SRPMS/sdata-0.2.2-1.src.rpm .
	@rm -rf rpmbuild
	@echo "SRPM created: sdata-0.2.2-1.src.rpm"
...
dsc: clean
	...
	@TARBALL_DIR="$(abspath $(dir $(lastword $(MAKEFILE_LIST)))/../Data/tarballs)"; \
	 ...
	 cd "$$TEMP_DIR" && tar czf "sdata_0.2.2.orig.tar.gz" "$$BASE_DIR"; \
	 cd "$$SRC_DIR" && dpkg-source -b .; \
	 mv "$$TEMP_DIR"/sdata_0.2.2* "$$CUR_DIR/" ; \
	 rm -rf "$$TEMP_DIR"
	@echo "Debian Source Package created (sdata_0.2.2-1.dsc, sdata_0.2.2.orig.tar.gz, etc.)"
...
slackware: clean
	...
	@TARBALL_DIR="$(abspath $(dir $(lastword $(MAKEFILE_LIST)))/../Data/tarballs)"; \
	 CUR_DIR=$$(pwd); \
	 TEMP_DIR=$$(mktemp -d); \
	 cp sdata-0.2.2.tar.gz "$$TEMP_DIR/"; \
	 cp slackware/* "$$TEMP_DIR/"; \
	 for tb in zipada-61.0.0.tar.gz xmlada-26.0.0.tar.gz mathpaqs-20260205.0.0.tar.gz; do \
	   cp "$$TARBALL_DIR/$$tb" "$$TEMP_DIR/"; \
	 done; \
	 cd "$$TEMP_DIR" && tar czf sdata-0.2.2-slackbuild.tar.gz *; \
	 mv "$$TEMP_DIR/sdata-0.2.2-slackbuild.tar.gz" "$$CUR_DIR/"; \
	 rm -rf "$$TEMP_DIR"
	@echo "SlackBuild package created: sdata-0.2.2-slackbuild.tar.gz"
...
pkg: build
	...
	@TEMP_DIR=$$(mktemp -d); \
	 ...
	 pkgbuild \
	   ...
	   sdata-0.2.2.pkg; \
	 rm -rf "$$TEMP_DIR"
	@echo "macOS package created: sdata-0.2.2.pkg"
